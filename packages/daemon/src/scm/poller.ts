...
export class ScmPoller {
  ...
  /**
   * The review verdict, plus how many threads are still unresolved.
   *
   * Only the latest review per author counts — an approval after a change request means the
   * reviewer is satisfied, and summing raw review rows would keep the task in the needs-you
   * set forever (§6 row 5).
   */
  async #reviewState(target: {
    ...
    let latest: ReviewState = 'none';
    for (const review of reviews) {
      const state = review.state.toUpperCase();
      if (state === 'CHANGES_REQUESTED') latest = 'changes_requested';
      else if (state === 'APPROVED') latest = 'approved';
      else if (state === 'COMMENTED' && latest === 'none') latest = 'commented';
    }

    const unresolved = latest === 'changes_requested' ? 1 : 0;
  ...
  async #changeRequestBodies(target: {
    ...
    const reviews = await this.#scm
      .get<(ReviewPayload & { body?: string | null })[]>(
        'GET /repos/{owner}/{repo}/pulls/{pull_number}/reviews',
        target,
      )
      .catch(() => null);
