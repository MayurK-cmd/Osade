...
canvas cell renderer over the substrate's endpoint protocol. Recon invalidated three of the four facts
...
- **Sources:** `visible` = the current viewport; `recent` / `recent_unwrapped` = the last N
  lines of scrollback (default 80, cap 1000); `detection` = the bottom-buffer slice the substrate's own
  detector reads.
- **Pollable?** Technically yes, badly. One request is one connection is one OS thread on
  the substrate (§4.2). At 30fps × 3 surfaces that is ~90 connection setups per second, each returning
...
Pane geometry and split ratios. Useful for reproducing the substrate's layout; no cell content.
...
scrollback editor over the JSON API. Any design leaning on them is designing against a substrate
...
| | |
| --- | --- |
| **Fragility** | **Highest.** Bincode enum tags are positional. A substrate build that inserts a `ServerMessage` variant shifts every tag after it, and the decoder misreads *silently* — wrong frame type, plausible garbage, no error. Only `EndpointControl` is contractually append-only (`wire.rs:1444-1448`). |
| **Version negotiation** | The handshake negotiates `generation` and four codec names, and deliberately does **not** compare build versions — that is the stability promise. But it covers the *envelope*, not the struct layouts inside it. A substrate whose `FrameData` gained a field still says "generation 1, codecs match" and then sends frames the decoder cannot read. **There is no handshake-level protection against exactly the failure this option is most likely to hit.** |
...
`queued → implementing → needs_input → awaiting_review` driven entirely by the substrate's detection,
with nothing polled. That full path was verified end-to-end against a live the substrate with **no
client attached and no cell ever rendered** (`SUBSTRATE-CONTRACT.md` §3.3). Option (a) is the
...
**3. What users need to read is not the substrate cells.** Verification output, diffs, gate payloads
and the activity line are all files or facts Osade owns on disk. The live terminal is the one
thing a real substrate client already renders perfectly, for free, with correct input.

**4. Option (a)'s failure mode is silent.** Since the endpoint handshake validates the envelope
rather than the payload structs, a substrate upgrade can leave the decoder reading garbage while
...
enough to feel live abuses the substrate's connection model.
...
- §23 open question 5 is closed: the `TerminalAnsi` + xterm.js fallback it named **does not
  exist** for endpoint clients (`RenderEncoding` is negotiated only on the substrate's private
...
- **New INVARIANT, recorded in §4.4 and §6.1.** Attaching any client marks panes seen, and
  ...
  (`backend/src/app/api_helpers.rs:100-106`). So "Open in the substrate" flips `done → idle` for that
...
1. A bincode decoder pinned to the vendored substrate, with a boot assertion that a known frame
